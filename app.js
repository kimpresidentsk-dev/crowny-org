...
const MALL_CATEGORIES = { fashion:'👗 패션', beauty:'💄 뷰티', food:'🍽️ 식품', digital:'💻 디지털', health:'💊 건강', home:'🏠 생활', handmade:'🧶 수공예', ticket:'🎫 티켓/쿠폰', other:'📦 기타' };
...
async function loadMallProducts() {
    ...
    try {
        const filterCat = document.getElementById('mall-filter-cat')?.value || 'all';
...
async function loadCreditInfo() {
    ...
    try {
        ...
        if (limitEl) limitEl.textContent = `${limit} FNC`;
        
        // 활성 대출
        const loans = await db.collection('loans').where('borrowerId','==',currentUser.uid).where('status','==','active').get();
...
async function requestLoan() {
    ...
    const days = parseInt(document.getElementById('loan-days').value) || 30;
    const token = document.getElementById('loan-token').value;
    
    if (!amount || !collateral) { alert('금액과 담보를 입력하세요'); return; }
    if (collateral < amount * 1.5) { alert(`담보 부족: 최소 ${Math.ceil(amount * 1.5)} CRNY 필요 (150%)`); return; }
    
    try {
        const wallets = await db.collection('users').doc(currentUser.uid).collection('wallets').limit(1).get();
        const bal = wallets.docs[0]?.data()?.balances || {};
        if ((bal.crny||0) < collateral) { alert('CRNY 담보 잔액 부족'); return; }
        
        // 담보 차감
        await wallets.docs[0].ref.update({ 'balances.crny': bal.crny - collateral });
        
        // 대출금 지급
        const tk = token.toLowerCase();
        await wallets.docs[0].ref.update({ [`balances.${tk}`]: (bal[tk]||0) + amount });
        
        const interest = Math.round(amount * 0.001 * days * 100) / 100;
        
        await db.collection('loans').add({
            borrowerId: currentUser.uid, borrowerEmail: currentUser.email,
            amount, token, collateral, collateralToken: 'CRNY',
            days, interestRate: 0.001, totalInterest: interest,
            repayAmount: amount + interest,
            dueDate: new Date(Date.now() + days * 86400000),
            status: 'active', createdAt: new Date()
        });
        
        alert(`💰 대출 완료!\n${amount} ${token} 지급\n담보: ${collateral} CRNY\n이자: ${interest} ${token}\n상환: ${amount + interest} ${token} (${days}일 후)`);
        loadCreditInfo(); loadUserWallet();
    } catch (e) { alert('실패: ' + e.message); }
}

async function loadLoanMarket() {
    const c = document.getElementById('loan-market');
    if (!c) return; c.innerHTML = '로딩...';
    try {
        const docs = await db.collection('loans').where('status','==','active').orderBy('createdAt','desc').limit(20).get();
        if (docs.empty) { c.innerHTML = '<p style="color:var(--accent);">활성 대출이 없습니다</p>'; return; }
        c.innerHTML = '';
        docs.forEach(d => { const x = d.data();
            const due = x.dueDate?.seconds ? new Date(x.dueDate.seconds*1000) : new Date(x.dueDate);
            const daysLeft = Math.max(0, Math.ceil((due - new Date()) / 86400000));
            c.innerHTML += `<div style="padding:0.8rem; background:var(--bg); border-radius:6px; margin-bottom:0.4rem; font-size:0.85rem;">
                <div style="display:flex; justify-content:space-between;"><span>💰 ${x.amount} ${x.token}</span><span>담보: ${x.collateral} CRNY</span></div>
                <div style="color:var(--accent); font-size:0.8rem;">이자: ${x.totalInterest} ${x.token} · 남은 기간: ${daysLeft}일</div>
            </div>`; });
    } catch (e) { c.innerHTML = e.message; }
}
