...
async function loadAdminWallet() {
    ...
    try {
        let walletRef = await db.collection('system_wallets').doc('admin').get();
        ...
        if (!walletRef.exists) {
            ...
            walletRef = await db.collection('system_wallets').doc('admin').get();
        }
        
        const data = walletRef.data();
        const balances = data.balances || {};
        
        document.getElementById('admin-wallet-info').innerHTML = `
            <div style="display:flex; gap:1rem; flex-wrap:wrap; margin-bottom:0.5rem;">
                <div style="background:#fff3e0; padding:0.6rem 1rem; border-radius:6px; text-align:center;">
                    ...
                    <strong style="font-size:1.2rem;">${(balances.crny || 0).toLocaleString()}</strong>
                </div>
                <div style="background:#e3f2fd; padding:0.6rem 1rem; border-radius:6px; text-align:center;">
                    <div style="font-size:0.7rem; color:#1565c0;">FNC</div>
                    <strong style="font-size:1.2rem;">${(balances.fnc || 0).toLocaleString()}</strong>
                </div>
                <div style="background:#e8f5e9; padding:0.6rem 1rem; border-radius:6px; text-align:center;">
                    ...
                    <strong style="font-size:1.2rem;">${(balances.crfn || 0).toLocaleString()}</strong>
                </div>
            </div>
...
async function adminSendToken() {
    ...
    try {
        // Admin 지갑 잔액 확인
        const adminWalletRef = await db.collection('system_wallets').doc('admin').get();
        const adminBalances = adminWalletRef.data()?.balances || {};
        
        if ((adminBalances[token] || 0) < amount) {
            alert(`Admin 지갑 ${token.toUpperCase()} 잔액 부족\n보유: ${adminBalances[token] || 0}\n필요: ${amount}`);
            return;
        }
        ...
        const targetWallet = wallets.docs[0];
        const targetBalances = targetWallet.data()?.balances || {};
        
        if (!window.confirm(`Admin 지갑에서 토큰 전송\n\n받는 사람: ${email}\n토큰: ${amount} ${token.toUpperCase()}\n\n진행하시겠습니까?`)) return;
        
        // Admin 지갑에서 차감
        await db.collection('system_wallets').doc('admin').update({
            [`balances.${token}`]: (adminBalances[token] || 0) - amount
        });
        
        // 받는 사람 지갑에 추가
        await db.collection('users').doc(targetUserId)
            .collection('wallets').doc(targetWallet.id)
            .update({
                [`balances.${token}`]: (targetBalances[token] || 0) + amount
            });
        ...
        alert(`✅ 전송 완료!\n${amount} ${token.toUpperCase()} → ${email}`);
...
function startLiveDataFeed() {
    ...
    console.log('✅ Databento 실시간 데이터 수신 시작 (1초 간격)');
}
...
async function fetchLiveTick() {
    try {
        ...
        // 최대 3600틱 보관 (1시간)
        if (window.liveTicks.length > 3600) {
