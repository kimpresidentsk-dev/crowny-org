// Cache Buster - Version 5.1 - Chart Tab System
// Global State
...
function updateTabSetting(field) {
    ...
    if (!tab) return;
    switch(field) {
        case 'symbol': tab.symbol = document.getElementById('tab-symbol').value; break;
        case 'chartType':
    ...
    updateChartLabel(); renderChartTabs(); saveChartTabs(); updateLiveCandleChart();
}
...
function aggregateTicksToTickCandles(ticks, ticksPerCandle) {
    ...
    for (const tick of ticks) {
        if (!cur || cnt >= ticksPerCandle) {
            ...
            cur = { time: tick.time, open: tick.price, high: tick.price, low: tick.price, close: tick.price, _tickCount: 1 };
            cnt = 1;
        } else {
            cur.high = Math.max(cur.high, tick.price);
            cur.low = Math.min(cur.low, tick.price);
            cur.close = tick.price;
            cur._tickCount++; cur.time = tick.time; cnt++;
        }
    }
    if (cur) candles.push(cur);
