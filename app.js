// Cache Buster - Version 5.0 - Trailing Stop + Close/Flatten
// Global State
...
function initAdminPage() {
    ...
    // 레벨별 섹션 표시
    if (hasLevel(4)) {
        document.getElementById('admin-wallet-section').style.display = 'block';
        loadAdminWallet();
    }
    if (hasLevel(6)) {
    ...
    if (hasLevel(3)) {
        document.getElementById('admin-participants-section').style.display = 'block';
...
async function swapTokens() {
    ...
    try {
        const wallets = await db.collection('users').doc(currentUser.uid).collection('wallets').limit(1).get();
        const bal = wallets.docs[0]?.data()?.balances || {};
        if ((bal[from]||0) < amount) { alert(`${from.toUpperCase()} 잔액 부족`); return; }
        
        await wallets.docs[0].ref.update({
            [`balances.${from}`]: bal[from] - amount,
            [`balances.${to}`]: (bal[to]||0) + amount
        });
        
        await db.collection('transactions').add({
            from: currentUser.uid, type: 'swap',
            fromToken: from.toUpperCase(), toToken: to.toUpperCase(),
            amount, fee: 0, timestamp: new Date()
        });
...
async function initTradingViewChart() {
    ...
    try {
        const chartHeight = window.innerWidth < 768 ? 350 : 450;
        const tzOffset = getTimezoneOffsetSeconds(selectedTimezone);
        
        // === NQ 메인 차트 (NinjaTrader 스타일) ===
        const chart = LightweightCharts.createChart(container, {
            ...
            grid: {
                vertLines: { color: '#1a1a2a', style: 1 },
                horzLines: { color: '#1a1a2a', style: 1 },
            },
            crosshair: {
        ...
        // 캔들 시리즈 (NinjaTrader 스타일 색상)
        const candleSeries = chart.addCandlestickSeries({
            upColor: '#0066cc',
            downColor: '#cc0000',
            borderUpColor: '#0066cc',
        ...
        // MA 라인 (두꺼운 프로 스타일)
        window.ma1Series = chart.addLineSeries({
            color: '#ffeb3b', lineWidth: 2, lineStyle: 0,
            priceLineVisible: false, lastValueVisible: true,
            title: 'MA5',
        });
        window.ma2Series = chart.addLineSeries({
            color: '#00bcd4', lineWidth: 2, lineStyle: 0,
            priceLineVisible: false, lastValueVisible: true,
            title: 'MA20',
        });
        window.ma3Series = chart.addLineSeries({
        ...
        if (mnqContainer) {
            ...
            const mnqChart = LightweightCharts.createChart(mnqContainer, {
                ...
                rightPriceScale: { borderColor: '#333', scaleMargins: { top: 0.05, bottom: 0.15 } },
                timeScale: {
            ...
            // MNQ 볼륨
            const mnqVolSeries = mnqChart.addHistogramSeries({
                color: '#26a69a', priceFormat: { type: 'volume' },
                priceScaleId: 'volume', scaleMargins: { top: 0.85, bottom: 0 },
            });
            mnqChart.priceScale('volume').applyOptions({ scaleMargins: { top: 0.85, bottom: 0 }, drawTicks: false, borderVisible: false });
            window.mnqVolumeSeries = mnqVolSeries;
            ...
            // MNQ MA 라인
            window.mnqMa1Series = mnqChart.addLineSeries({ color: '#ffeb3b', lineWidth: 2, priceLineVisible: false, lastValueVisible: true, title: 'MA5' });
            window.mnqMa2Series = mnqChart.addLineSeries({ color: '#00bcd4', lineWidth: 2, priceLineVisible: false, lastValueVisible: true, title: 'MA20' });
            window.mnqMa3Series = mnqChart.addLineSeries({ color: '#e040fb', lineWidth: 2, priceLineVisible: false, lastValueVisible: true, title: 'MA60' });
            
            window.mnqChart = mnqChart;
            window.mnqCandleSeries = mnqCandleSeries;
        }
        
        console.log('📊 프로 차트 준비 완료 (NinjaTrader 스타일)');
...
async function fetchLiveTick() {
    try {
        ...
        if (window.liveTicks.length > 0) {
            ...
            // 이전 틱 대비 30포인트 이상 변동 시 무시
            if (diff > 30) {
                console.warn(`⚠️ 스파이크 필터: ${lastPrice} → ${data.price} (${diff.toFixed(1)}pt 변동)`);
                return;
            }
        }
...
// MA 라인 업데이트 (NQ/MNQ 독립)
function updateMALines(candles) {
    // NQ MA
    const nqMa1P = parseInt(document.getElementById('nq-ma1-period')?.value) || 5;
    const nqMa2P = parseInt(document.getElementById('nq-ma2-period')?.value) || 20;
    const nqMa3P = parseInt(document.getElementById('nq-ma3-period')?.value) || 60;
    const nqMa1Show = document.getElementById('nq-ma1-show')?.checked !== false;
    const nqMa2Show = document.getElementById('nq-ma2-show')?.checked !== false;
    const nqMa3Show = document.getElementById('nq-ma3-show')?.checked !== false;
    
    const nqMa1Data = calculateMA(candles, nqMa1P);
    const nqMa2Data = calculateMA(candles, nqMa2P);
    const nqMa3Data = calculateMA(candles, nqMa3P);
    
    if (window.ma1Series) { window.ma1Series.setData(nqMa1Show ? nqMa1Data : []); }
    if (window.ma2Series) { window.ma2Series.setData(nqMa2Show ? nqMa2Data : []); }
    if (window.ma3Series) { window.ma3Series.setData(nqMa3Show ? nqMa3Data : []); }
    
    // MNQ MA (독립 설정)
    const mnqMa1P = parseInt(document.getElementById('mnq-ma1-period')?.value) || 5;
    const mnqMa2P = parseInt(document.getElementById('mnq-ma2-period')?.value) || 20;
    const mnqMa3P = parseInt(document.getElementById('mnq-ma3-period')?.value) || 60;
    const mnqMa1Show = document.getElementById('mnq-ma1-show')?.checked !== false;
    const mnqMa2Show = document.getElementById('mnq-ma2-show')?.checked !== false;
    const mnqMa3Show = document.getElementById('mnq-ma3-show')?.checked !== false;
    
    const mnqMa1Data = calculateMA(candles, mnqMa1P);
    const mnqMa2Data = calculateMA(candles, mnqMa2P);
    const mnqMa3Data = calculateMA(candles, mnqMa3P);
    
    if (window.mnqMa1Series) { window.mnqMa1Series.setData(mnqMa1Show ? mnqMa1Data : []); }
    if (window.mnqMa2Series) { window.mnqMa2Series.setData(mnqMa2Show ? mnqMa2Data : []); }
    if (window.mnqMa3Series) { window.mnqMa3Series.setData(mnqMa3Show ? mnqMa3Data : []); }
...
function updateMAInfoDisplay() {
    const nqInfo = document.getElementById('nq-ma-info');
    const mnqInfo = document.getElementById('mnq-ma-info');
    
    if (nqInfo) {
        const parts = [];
        if (document.getElementById('nq-ma1-show')?.checked) parts.push(document.getElementById('nq-ma1-name')?.value || 'MA1');
        if (document.getElementById('nq-ma2-show')?.checked) parts.push(document.getElementById('nq-ma2-name')?.value || 'MA2');
        if (document.getElementById('nq-ma3-show')?.checked) parts.push(document.getElementById('nq-ma3-name')?.value || 'MA3');
        nqInfo.textContent = parts.join(' · ');
    }
    if (mnqInfo) {
        const parts = [];
        if (document.getElementById('mnq-ma1-show')?.checked) parts.push(document.getElementById('mnq-ma1-name')?.value || 'MA1');
        if (document.getElementById('mnq-ma2-show')?.checked) parts.push(document.getElementById('mnq-ma2-name')?.value || 'MA2');
        if (document.getElementById('mnq-ma3-show')?.checked) parts.push(document.getElementById('mnq-ma3-name')?.value || 'MA3');
        mnqInfo.textContent = parts.join(' · ');
    }
}
...
function applyMASettings() {
    // NQ 설정
    const nqMa1Color = document.getElementById('nq-ma1-color')?.value || '#ffeb3b';
    const nqMa2Color = document.getElementById('nq-ma2-color')?.value || '#00bcd4';
    const nqMa3Color = document.getElementById('nq-ma3-color')?.value || '#e040fb';
    const nqMa1Name = document.getElementById('nq-ma1-name')?.value || 'MA1';
    const nqMa2Name = document.getElementById('nq-ma2-name')?.value || 'MA2';
    const nqMa3Name = document.getElementById('nq-ma3-name')?.value || 'MA3';
    const nqMa1Period = document.getElementById('nq-ma1-period')?.value || '5';
    const nqMa2Period = document.getElementById('nq-ma2-period')?.value || '20';
    const nqMa3Period = document.getElementById('nq-ma3-period')?.value || '60';
    const nqMa1Show = document.getElementById('nq-ma1-show')?.checked !== false;
    const nqMa2Show = document.getElementById('nq-ma2-show')?.checked !== false;
    const nqMa3Show = document.getElementById('nq-ma3-show')?.checked !== false;
    const nqLabelShow = document.getElementById('nq-ma-label-show')?.checked !== false;
    
    if (window.ma1Series) window.ma1Series.applyOptions({ color: nqMa1Color, title: nqLabelShow ? nqMa1Name : '', lastValueVisible: nqLabelShow });
    if (window.ma2Series) window.ma2Series.applyOptions({ color: nqMa2Color, title: nqLabelShow ? nqMa2Name : '', lastValueVisible: nqLabelShow });
    if (window.ma3Series) window.ma3Series.applyOptions({ color: nqMa3Color, title: nqLabelShow ? nqMa3Name : '', lastValueVisible: nqLabelShow });
    
    // MNQ 설정
    const mnqMa1Color = document.getElementById('mnq-ma1-color')?.value || '#ffeb3b';
    const mnqMa2Color = document.getElementById('mnq-ma2-color')?.value || '#00bcd4';
    const mnqMa3Color = document.getElementById('mnq-ma3-color')?.value || '#e040fb';
    const mnqMa1Name = document.getElementById('mnq-ma1-name')?.value || 'MA1';
    const mnqMa2Name = document.getElementById('mnq-ma2-name')?.value || 'MA2';
    const mnqMa3Name = document.getElementById('mnq-ma3-name')?.value || 'MA3';
    const mnqMa1Period = document.getElementById('mnq-ma1-period')?.value || '5';
    const mnqMa2Period = document.getElementById('mnq-ma2-period')?.value || '20';
    const mnqMa3Period = document.getElementById('mnq-ma3-period')?.value || '60';
    const mnqMa1Show = document.getElementById('mnq-ma1-show')?.checked !== false;
    const mnqMa2Show = document.getElementById('mnq-ma2-show')?.checked !== false;
    const mnqMa3Show = document.getElementById('mnq-ma3-show')?.checked !== false;
    const mnqLabelShow = document.getElementById('mnq-ma-label-show')?.checked !== false;
    
    if (window.mnqMa1Series) window.mnqMa1Series.applyOptions({ color: mnqMa1Color, title: mnqLabelShow ? mnqMa1Name : '', lastValueVisible: mnqLabelShow });
    if (window.mnqMa2Series) window.mnqMa2Series.applyOptions({ color: mnqMa2Color, title: mnqLabelShow ? mnqMa2Name : '', lastValueVisible: mnqLabelShow });
    if (window.mnqMa3Series) window.mnqMa3Series.applyOptions({ color: mnqMa3Color, title: mnqLabelShow ? mnqMa3Name : '', lastValueVisible: mnqLabelShow });
    
    // localStorage에 저장
    const settings = {
        nq: {
            ma1: { color: nqMa1Color, name: nqMa1Name, period: nqMa1Period, show: nqMa1Show },
            ma2: { color: nqMa2Color, name: nqMa2Name, period: nqMa2Period, show: nqMa2Show },
            ma3: { color: nqMa3Color, name: nqMa3Name, period: nqMa3Period, show: nqMa3Show },
            labelShow: nqLabelShow
        },
        mnq: {
            ma1: { color: mnqMa1Color, name: mnqMa1Name, period: mnqMa1Period, show: mnqMa1Show },
            ma2: { color: mnqMa2Color, name: mnqMa2Name, period: mnqMa2Period, show: mnqMa2Show },
            ma3: { color: mnqMa3Color, name: mnqMa3Name, period: mnqMa3Period, show: mnqMa3Show },
            labelShow: mnqLabelShow
        }
    };
    ...
    console.log('📈 MA 설정 적용 + 저장 완료');
}

// localStorage에서 MA 설정 로드 → HTML 입력에 반영
function loadMASettings() {
    try {
        ...
        if (s.mnq) {
            if (s.mnq.ma1) {
                const el = document.getElementById('mnq-ma1-color'); if (el) el.value = s.mnq.ma1.color;
                const n = document.getElementById('mnq-ma1-name'); if (n) n.value = s.mnq.ma1.name;
                const p = document.getElementById('mnq-ma1-period'); if (p) p.value = s.mnq.ma1.period;
                const sh = document.getElementById('mnq-ma1-show'); if (sh) sh.checked = s.mnq.ma1.show;
            }
            if (s.mnq.ma2) {
                const el = document.getElementById('mnq-ma2-color'); if (el) el.value = s.mnq.ma2.color;
                const n = document.getElementById('mnq-ma2-name'); if (n) n.value = s.mnq.ma2.name;
                const p = document.getElementById('mnq-ma2-period'); if (p) p.value = s.mnq.ma2.period;
                const sh = document.getElementById('mnq-ma2-show'); if (sh) sh.checked = s.mnq.ma2.show;
            }
            if (s.mnq.ma3) {
                const el = document.getElementById('mnq-ma3-color'); if (el) el.value = s.mnq.ma3.color;
                const n = document.getElementById('mnq-ma3-name'); if (n) n.value = s.mnq.ma3.name;
                const p = document.getElementById('mnq-ma3-period'); if (p) p.value = s.mnq.ma3.period;
                const sh = document.getElementById('mnq-ma3-show'); if (sh) sh.checked = s.mnq.ma3.show;
            }
            const lb = document.getElementById('mnq-ma-label-show'); if (lb) lb.checked = s.mnq.labelShow;
        }
        
        console.log('📈 저장된 MA 설정 로드 완료');
    } catch(e) { console.warn('MA 설정 로드 실패:', e); }
}
    ...
    openTrades.forEach((trade, index) => {
        ...
        if (trade.stopLoss || trade.takeProfit) {
            ...
            const trailBadge = (ts && ts.enabled) 
                ? `<span style="display:inline-block; background:${ts.activated ? '#ff9800' : '#666'}; color:white; font-size:0.65rem; padding:1px 5px; border-radius:3px; margin-left:4px;">${ts.activated ? '🔄 TRAIL' : '⏳ 대기'}</span>` 
                : '';
            
            slTPHTML = `
                <div style="display:flex; gap:0.8rem; margin-top:0.5rem; font-size:0.8rem; flex-wrap:wrap; align-items:center;">
                    ${trade.stopLoss ? `<span style="color:#ff4444;">SL: ${trade.stopLoss.toFixed(2)}</span>` : ''}
                    ${trade.takeProfit ? `<span style="color:#00cc00;">TP: ${trade.takeProfit.toFixed(2)}</span>` : ''}
                    ${trailBadge}
                </div>
            `;
            ...
            if (ts && ts.enabled && ts.activated) {
                ...
                slTPHTML += `
                    <div style="font-size:0.7rem; color:#ff9800; margin-top:0.3rem;">
                        🔄 최${trade.side === 'BUY' ? '고' : '저'}가: ${hwm.toFixed(2)} | 거리: ${ts.distance}pt
                    </div>
                `;
            }
        }
        ...
        div.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:start;">
                ...
                <div style="display:flex; flex-direction:column; gap:0.4rem;">
                    ...
                    ${(trade.trailingStop && trade.trailingStop.enabled) ? `
                        <button onclick="toggleTrailingForTrade(${actualIndex})" style="background:${trade.trailingStop.activated ? '#ff9800' : '#666'}; color:white; border:none; padding:0.3rem 0.6rem; border-radius:4px; cursor:pointer; font-size:0.65rem;">
                            ${trade.trailingStop.activated ? '🔄 ON' : '⏸ OFF'}
                        </button>
                    ` : `
                        <button onclick="enableTrailingForTrade(${actualIndex})" style="background:#444; color:#aaa; border:none; padding:0.3rem 0.6rem; border-radius:4px; cursor:pointer; font-size:0.65rem;">
                            +트레일
                        </button>
                    `}
                </div>
...
function drawPositionLinesLW() {
    ...
    if (window.mnqPositionLines && window.mnqCandleSeries) {
        window.mnqPositionLines.forEach(line => {
            try {
                window.mnqCandleSeries.removePriceLine(line);
            } catch (e) {}
        });
    }
    window.positionLines = [];
    window.mnqPositionLines = [];
    ...
    if (openTrades.length === 0) {
        console.log('📊 오픈 포지션 없음 - 라인 제거 완료');
        return;
    }
    
    openTrades.forEach((trade) => {
        // 어느 차트에 그릴지 결정
        const isNQ = trade.contract === 'NQ';
        const targetSeries = isNQ ? window.candleSeries : (window.mnqCandleSeries || window.candleSeries);
        const targetLines = isNQ ? window.positionLines : window.mnqPositionLines;
        
        if (!targetSeries) return;
        
        // 진입가 라인
        const entryLine = targetSeries.createPriceLine({
        ...
        targetLines.push(entryLine);
        
        // 손절 라인 (트레일링 활성 시 색상 다르게)
        if (trade.stopLoss) {
            const isTrailing = trade.trailingStop?.activated;
            const slLine = targetSeries.createPriceLine({
            ...
            targetLines.push(slLine);
        }
        
        // 익절 라인
        if (trade.takeProfit) {
            const tpLine = targetSeries.createPriceLine({
