...
async function loadAdminWallet() {
    ...
    container.innerHTML = '<p style="color:var(--accent);">🔄 온체인 잔액 조회 중...</p>';
    
    try {
        // Firestore에서 관리자의 Polygon 지갑 주소 가져오기
        const wallets = await db.collection('users').doc(currentUser.uid)
            .collection('wallets').limit(1).get();
        
        if (wallets.empty) {
            container.innerHTML = '<p style="color:red;">지갑을 찾을 수 없습니다</p>';
            return;
        }
        ...
        if (!adminAddress) {
            container.innerHTML = '<p style="color:red;">지갑 주소가 없습니다</p>';
            return;
        }
