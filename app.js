...
function updateRiskGaugeUI() {
    ...
    const actualDailyLimit = myParticipation.dailyLossLimit || Math.abs(RISK_CONFIG.dailyLossLimit);
    const actualCumulativeLimit = myParticipation.maxDrawdown || Math.abs(RISK_CONFIG.cumulativeLossLimit);
...
async function checkDailyLossLimit() {
    ...
    try {
        ...
        if (freshDoc.exists) {
            ...
            // 관리자가 변경 가능한 필드만 동기화
            if (fresh.dailyLossLimit !== undefined) myParticipation.dailyLossLimit = fresh.dailyLossLimit;
            if (fresh.maxDrawdown !== undefined) myParticipation.maxDrawdown = fresh.maxDrawdown;
    ...
    const limitValue = myParticipation.dailyLossLimit || Math.abs(RISK_CONFIG.dailyLossLimit);
...
async function adminAdjustDailyLimit(participantId, challengeId) {
    ...
    try {
        ...
        await db.collection('prop_challenges').doc(challengeId)
            ...
            .update({ dailyLossLimit: parseFloat(newLimit) });
...
async function adminAdjustMaxDrawdown(participantId, challengeId) {
    ...
    try {
        ...
        await db.collection('prop_challenges').doc(challengeId)
            .collection('participants').doc(participantId)
            .update({ maxDrawdown: parseFloat(newDD) });
