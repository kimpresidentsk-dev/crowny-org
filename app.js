...
// Databento 실시간 서버에서 NQ 데이터 가져오기 (차트용)
async function fetchRealNQData() {
    try {
        ...
        if (data && data.price) {
            ...
            console.log('✅ Databento 실시간 NQ 가격:', currentPrice);
            // 현재 가격 기반으로 초기 차트 데이터 생성
            return generateSampleDataFromPrice(currentPrice);
        }
...
// 실시간 가격 기반 초기 차트 데이터
function generateSampleDataFromPrice(basePrice) {
    const candles = [];
    const volume = [];
    const now = Math.floor(Date.now() / 1000);
    
    for (let i = 50; i >= 0; i--) {
        const time = now - i * 300; // 5분 간격
...
function startRealPriceUpdates() {
    ...
    console.log('✅ Databento 실시간 가격 업데이트 시작 (3초 간격)');
}
