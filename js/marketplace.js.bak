...
const ORDER_STATUS_LABELS = { paid:'💰 결제완료', shipping:'🚚 배송중', delivered:'✅ 배송완료', cancelled:'❌ 취소' };
            ...
            revSnap.forEach(r => {
                ...
                reviewsHtml += `<div style="background:var(--bg); padding:0.6rem; border-radius:6px; margin-bottom:0.4rem;">
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                        <span style="font-size:0.8rem; font-weight:600;">${rv.buyerEmail?.split('@')[0] || '구매자'}</span>
        ...
        try {
            ...
            if (!myOrders.empty) {
                ...
                if (existingReview.empty) {
                    reviewBtnHtml = `<button onclick="writeReview('${id}')" style="background:#ff9800; color:white; border:none; padding:0.6rem; border-radius:8px; cursor:pointer; font-weight:600; width:100%; margin-top:0.5rem;">⭐ 리뷰 작성</button>`;
                }
            }
        } catch(e) {}
    }
...
async function writeReview(productId) {
    const ratingStr = await showPromptModal('별점', '1~5점을 입력하세요', '5');
    const rating = parseInt(ratingStr);
    if (!rating || rating < 1 || rating > 5) { showToast('1~5 사이 점수를 입력하세요', 'warning'); return; }
    const comment = await showPromptModal('리뷰', '한줄 리뷰를 작성하세요 (선택)', '');
    try {
            ...
            c.innerHTML += `<div style="padding:0.6rem; background:var(--bg); border-radius:6px; margin-bottom:0.4rem; font-size:0.85rem;">
                <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:0.3rem;">
                    ...
                    <div style="display:flex; gap:0.3rem;">
                        <button onclick="editProduct('${d.id}')" style="background:#2196f3; color:white; border:none; padding:0.2rem 0.5rem; border-radius:4px; cursor:pointer; font-size:0.75rem;">✏️ 수정</button>
                        <button onclick="toggleProduct('${d.id}','${x.status}')" style="background:${x.status==='active'?'#999':'#4CAF50'}; color:white; border:none; padding:0.2rem 0.5rem; border-radius:4px; cursor:pointer; font-size:0.75rem;">${x.status==='active'?'⏸ 비활성':'▶ 활성'}</button>
...
async function editProduct(id) {
    ...
    if (!doc.exists) return;
    const p = doc.data();
    const newPrice = await showPromptModal('가격 수정', `현재 가격: ${p.price} ${p.priceToken}`, String(p.price));
    if (newPrice === null) return;
    const newStock = await showPromptModal('재고 수정', `현재 재고: ${p.stock}`, String(p.stock));
    if (newStock === null) return;
    const newDesc = await showPromptModal('설명 수정', '상품 설명', p.description || '');
...
async function deleteProduct(id) {
    if (!await showConfirmModal('상품 삭제', '이 상품을 삭제하시겠습니까? 복구할 수 없습니다.')) return;
    try {
        await db.collection('products').doc(id).delete();
        showToast('🗑️ 상품 삭제 완료', 'success');
        loadMyProducts();
...
async function loadSellerOrders() {
    ...
    try {
        ...
        o.forEach(d => {
            ...
            const statusColor = ORDER_STATUS_COLORS[x.status] || 'var(--accent)';
            const nextActions = [];
            if (x.status === 'paid') nextActions.push(`<button onclick="updateOrderStatus('${d.id}','shipping')" style="background:#2196f3; color:white; border:none; padding:0.2rem 0.5rem; border-radius:4px; cursor:pointer; font-size:0.75rem;">🚚 배송처리</button>`);
            if (x.status === 'shipping') nextActions.push(`<button onclick="updateOrderStatus('${d.id}','delivered')" style="background:#4CAF50; color:white; border:none; padding:0.2rem 0.5rem; border-radius:4px; cursor:pointer; font-size:0.75rem;">✅ 배송완료</button>`);
            c.innerHTML += `<div style="padding:0.6rem; background:var(--bg); border-radius:6px; margin-bottom:0.4rem; font-size:0.85rem;">
                <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:0.3rem;">
...
async function updateOrderStatus(orderId, newStatus) {
    ...
    if (!await showConfirmModal('주문 상태 변경', `${label}(으)로 변경하시겠습니까?`)) return;
    try {
                ...
                <div style="background:white; border-radius:12px; overflow:hidden; box-shadow:0 2px 8px rgba(0,0,0,0.08); cursor:pointer;" onclick="showCampaignDetail('${d.id}')">
                    ...
                    <div style="padding:1rem;">
                        ...
                        <div style="display:flex; gap:0.5rem; margin-top:0.8rem;">
                            <button onclick="event.stopPropagation(); donateCampaign('${d.id}')" style="background:#4CAF50; color:white; border:none; padding:0.6rem; border-radius:6px; cursor:pointer; flex:1; font-weight:700;">💝 기부하기</button>
                            ${isCreator ? `<button onclick="event.stopPropagation(); closeCampaign('${d.id}')" style="background:#e53935; color:white; border:none; padding:0.6rem; border-radius:6px; cursor:pointer; font-weight:700; font-size:0.8rem;">🔒 종료</button>` : ''}
                        </div>
                    </div>
                </div>`;
        });
        ...
        docs.forEach(d => { const x = d.data(); const pct = Math.min(100, Math.round((x.invested / x.goal)*100));
            ...
            c.innerHTML += `<div style="background:var(--bg); padding:1rem; border-radius:8px; margin-bottom:0.8rem;">
                ...
                <div style="display:flex; gap:0.5rem; margin-top:0.5rem;">
                    <button onclick="investEnergy('${d.id}')" style="background:#ff9800; color:white; border:none; padding:0.5rem; border-radius:6px; cursor:pointer; flex:1;">☀️ 투자하기</button>
                    ${isAdmin ? `<button onclick="distributeEnergyReturns('${d.id}')" style="background:#1976D2; color:white; border:none; padding:0.5rem; border-radius:6px; cursor:pointer; flex:1; font-size:0.8rem;">📊 수익 배분</button>` : ''}
                </div>
            </div>`; });
    } catch (e) { c.innerHTML = e.message; }
}
    ...
    try {
        ...
        docs.forEach(d => { const x = d.data();
            c.innerHTML += `<div onclick="viewArtistDetail('${d.id}')" style="background:white; border-radius:10px; overflow:hidden; text-align:center; box-shadow:0 2px 8px rgba(0,0,0,0.08); cursor:pointer;">
                ...
                <button onclick="event.stopPropagation(); supportArtist('${d.id}')" style="background:#E91E63; color:white; border:none; padding:0.4rem 0.8rem; border-radius:6px; cursor:pointer; margin-top:0.4rem; font-size:0.8rem;">💖 후원</button>
                </div></div>`; });
    } catch (e) { c.innerHTML = e.message; }
}
...
async function approveInsurance(id) {
    ...
    try {
        const userDoc = await db.collection('users').doc(currentUser.uid).get();
        if ((userDoc.data()?.adminLevel || 0) < 2) { showToast('관리자 권한이 필요합니다', 'error'); return; }
        const doc = await db.collection('insurance_requests').doc(id).get();
        if (!doc.exists) return;
        const req = doc.data();
        if (req.status !== 'pending') { showToast('이미 처리된 요청입니다', 'info'); return; }
