...
function applyTradingPermissions() {
    ...
    if (maxLabel) maxLabel.textContent = `(최대 ${max})`;
...
const DEFAULT_TIERS = {
    A: { deposit: 100, account: 100000, liquidation: 3000, profitThreshold: 1000, withdrawUnit: 1000, label: '🅰️ 교육 기본' },
    B: { deposit: 200, account: 150000, liquidation: 5000, profitThreshold: 1500, withdrawUnit: 1000, label: '🅱️ 중급' },
    C: { deposit: 500, account: 300000, liquidation: 10000, profitThreshold: 3000, withdrawUnit: 1000, label: '🅲 프로' },
};
    ...
    el.innerHTML = `
        ...
        <div style="margin-bottom:0.5rem;">
            <div style="display:flex; justify-content:space-between; font-size:0.7rem; margin-bottom:0.15rem;">
                <span>🛡️ 생존</span>
                <span style="color:${lifeColor};">-$${cfg.liquidation} 까지 $${(cfg.liquidation + pnl).toFixed(0)} 남음</span>
            </div>
        ...
        <div style="margin-bottom:0.5rem;">
            <div style="display:flex; justify-content:space-between; font-size:0.7rem; margin-bottom:0.15rem;">
                <span>📈 수익 → CRTD</span>
                <span style="color:${profitColor};">${pnl >= cfg.profitThreshold ? `🟢 변환구간 (+$${profitAboveThreshold.toFixed(0)} = ${Math.floor(profitAboveThreshold)} CRTD)` : `+$${cfg.profitThreshold} 도달 시 활성`}</span>
            </div>
            <div style="background:rgba(255,255,255,0.1); height:5px; border-radius:3px;">
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; font-size:0.72rem; padding-top:0.3rem; border-top:1px solid rgba(255,255,255,0.1);">
            <span>💰 인출 가능: <strong style="color:${withdrawable > 0 ? '#00ff88' : '#888'};">${withdrawable} CRTD</strong> (${cfg.withdrawUnit}단위)</span>
            <span>기인출: ${totalWithdrawn}</span>
        </div>
...
async function loadTradingDashboard() {
    ...
    if (myParticipation) {
        ...
        const productText = productParts.length > 0 ? productParts.join(' + ') : '미설정';
        const rulesEl = document.getElementById('prop-rules-display');
        const cfg = getCRTDConfig();
        if (rulesEl) {
            rulesEl.innerHTML = `
                <p><strong>💎 ${cfg.tier}등급:</strong> ${cfg.deposit} CRTD 참가비</p>
                <p><strong>💰 가상 계좌:</strong> $${(p.initialBalance || 100000).toLocaleString()} USD</p>
                <p><strong>📊 거래 가능:</strong> ${productText}</p>
                <p><strong>🔴 일일 한도:</strong> -$${p.dailyLossLimit || 100} 손실 시 당일 중단</p>
                <p><strong>💀 청산:</strong> -$${cfg.liquidation.toLocaleString()} 손실 시 계좌 종료 (${cfg.deposit} CRTD 소멸)</p>
                <p><strong>📈 수익 변환:</strong> +$${cfg.profitThreshold.toLocaleString()} 초과분 → 1:1 CRTD</p>
                <p><strong>💰 인출:</strong> ${cfg.withdrawUnit.toLocaleString()} CRTD 단위</p>
            `;
        }
